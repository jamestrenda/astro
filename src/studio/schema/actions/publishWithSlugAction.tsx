      ...
      onHandle: async () => {
        ...
        if (slug.current === publishedSlug?.current) {
          ...
          const existingRedirects: Redirect[] =
            await client.fetch(REDIRECTS_QUERY);
          ...
          if (existingRedirect) {
            ...
            let id = await client.fetch(REDIRECT_SETTINGS_ID_QUERY);

            if (!id) {
              // if there is no redirectSettings document, create one
              const doc = {
                _id: redirectSettings.name,
                _type: redirectSettings.name,
              };
